...
- **Обновление с более ранних сборок требует пересоздания рабочего профиля**
  ...
  папок SAF. Это про прошлую миграцию, а не про платформенный запрет: на стенде
  проверено, что владение профилем передается другому приложению и обратно, и
  профиль переживает даже полную переустановку Gatekeeper с другой подписью
  (`.ai/ui-redesign/platform-walls-research.md`, раздел 9a). Чтобы этим можно
  было пользоваться, нужен отдельный APK-компаньон, который примет владение на
  время переустановки; в продукте его пока нет.
