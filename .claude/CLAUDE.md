...
## Codebase Intelligence for zindan (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-23 (commit 766f06a). Confidence: 100%.
...
### Architecture
**Files:** 94 | **Lines:** 16023 | **Import cycles:** 1
zindan is a kotlin codebase of 94 files. Ranked by PageRank over the import graph: the files most of the codebase ultimately depends on. ---
*Built from the code's structure. It states what is there, not why it is that
way.

### Key modules
- `app/src/main/java/io/gatekeeper/util` — app/src/main/java/io/gatekeeper/util
**Language:** kotlin | **Files:** 34 | **Public symbols:** 553 / 627
Covers the 34 source files in…
- `app/src/main/java/io/gatekeeper/ui` — app/src/main/java/io/gatekeeper/ui
**Language:** kotlin | **Files:** 11 | **Public symbols:** 335 / 460
Covers the 11 source files in…
- `app/src/main/java/io/gatekeeper/services` — app/src/main/java/io/gatekeeper/services
**Language:** kotlin | **Files:** 8 | **Public symbols:** 137 / 189
Covers the 8 source files in…
- `app/src/main/java/io/gatekeeper` — app/src/main/java/io/gatekeeper · app/src/main/java/io/gatekeeper/receivers
**Language:** kotlin | **Files:** 6 | **Public symbols:** 28 /…

### Files that need care (bug-fix history first, then churn — check `get_risk` before editing)
- `app/src/main/java/io/gatekeeper/ui/MainActivity.kt` — 4 commits/90d
- `app/src/main/java/io/gatekeeper/ui/DummyActivity.kt` — 3 commits/90d
- `app/src/main/java/io/gatekeeper/ui/SettingsFragment.kt` — 3 commits/90d
- `app/src/main/java/io/gatekeeper/util/Utility.kt` — 4 commits/90d
- `tools/testbench.sh` — 3 commits/90d

### Code health
Three co-equal signals: defect risk 7.51/10 avg, hotspot health 5.55/10 (stable), worst `app/src/main/java/io/gatekeeper/ui/DummyActivity.kt` at 3.55/10 · maintainability 8.2/10 · performance risk 2 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `app/src/main/java/io/gatekeeper/ui/DummyActivity.kt` — untested hotspot — impact −2.0
- `app/src/main/java/io/gatekeeper/ui/MainActivity.kt` — low cohesion (MainActivity) — impact −1.0
- `app/src/main/java/io/gatekeeper/ui/SettingsFragment.kt` — low cohesion (SettingsFragment) — impact −0.6
