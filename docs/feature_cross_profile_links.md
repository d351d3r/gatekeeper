...
- **Проверено на эмуляторе API 36 (12.09.2026).** Служебные фильтры на
  кастомные action'ы работают: личный профиль видит компоненты рабочего и
  показывает резолвер (проверено `REFRESH_MAIN_APP_LIST`, ушло через
  `ForwardIntentToManagedProfile`). А вот `ACTION_VIEW` + BROWSABLE на
  `http(s)` **не резолвится кросс-профильно**: с отключённым браузером в
  личном профиле интент падает с `START_INTENT_NOT_RESOLVED (-91)`, рабочий
  браузер не предлагается. То есть доменные правила C2 на текущей платформе
  не имеют наблюдаемого эффекта — это то самое ограничение B6 «ссылки из
  личного не открываются в рабочем». Правила остаются в настройках и бэкапе
  как задекларированное намерение; механика применения в коде (единый
  владелец — `enforceWorkProfilePolicies`) работает и не ломает релей.
  Железная проверка на устройстве пользователя всё равно нужна: вендорские
  прошивки могут вести себя иначе, чем AOSP-эмулятор.
