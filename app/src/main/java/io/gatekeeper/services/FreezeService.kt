    ...
    private val freezeWork: AlarmManager.OnAlarmListener = AlarmManager.OnAlarmListener {
        synchronized(FreezeService::class.java) {
            ...
            if (scope == ScreenLockFreezeScope.SESSION) {
                freezeSessionApps()
                // Сеанс закрыт заморозкой -- слушать экран больше нечего.
                stopSelf()
            } else {
                ...
                // Списловые области слушают экран постоянно: после первой заморозки
                // блокировать его всё ещё нужно. Останов просит SYNC_FREEZE_SERVICE.
            }
        }
    }
