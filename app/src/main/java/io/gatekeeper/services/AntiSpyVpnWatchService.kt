...
class AntiSpyVpnWatchService : Service() {
    ...
    private fun registerFreezeCompleteReceiver() {
        ...
        val receiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                if (intent.action != CrossProfileScheduler.ACTION_VPN_BATCH_FREEZE_SESSION_COMPLETE) {
                    return
                }
        ...
        freezeCompleteReceiver = registerLocalReceiver(
            receiver,
            IntentFilter(CrossProfileScheduler.ACTION_VPN_BATCH_FREEZE_SESSION_COMPLETE),
            "freeze-complete",
        )
    }
    ...
    private fun postVpnStateAlert(textResId: Int) {
        ...
        Notifications.postUserAlert(this, VPN_STATE_NOTIFICATION_ID, title, text)
    }
    ...
    private fun freezeFromMainWatcher() {
        ...
        CrossProfileScheduler.requestVpnBatchFreeze(this)
    ...
    private fun freezeInWorkProfile() {
        ...
        if (list.isEmpty()) {
            Log.w(TAG, "freeze in work: auto-freeze list is empty")
            postFreezeProblem(
                getString(R.string.notif_freeze_list_empty_title),
                getString(R.string.notif_freeze_list_empty_text),
            )
            return
        }
        ...
        if (stillVisible > 0) {
            postFreezeProblem(
                getString(R.string.notif_freeze_incomplete_title),
                getString(R.string.notif_freeze_incomplete_text, stillVisible),
            )
            return
        }
        vpnFreezeDoneForSession = true
        CrossProfileScheduler.notifyVpnBatchFreezeSessionComplete(this, frozen > 0)
    }

    /** Отсчет до заморозки виден пользователю и отменяется кнопкой -- иначе реакция мгновенная. */
    private fun postFreezePendingAlert() {
        val notification = Notifications.buildUserAlertNotification(
            this,
            getString(R.string.anti_spy_monitor_notification_title),
    ...
    /** Заморозка не состоялась: это решение пользователя, а не диагностика сторожа. */
    private fun postFreezeProblem(title: String, text: String) {
        val id = if (isMainProfileWatcher()) WARN_MAIN_NOTIFICATION_ID else WARN_WORK_NOTIFICATION_ID
        Notifications.postUserAlert(this, id, title, text, Notifications.CHANNEL_WARNING)
    }
    ...
    companion object {
        ...
        private const val WARN_MAIN_NOTIFICATION_ID = 0xe49da
        private const val WARN_WORK_NOTIFICATION_ID = 0xe49db
