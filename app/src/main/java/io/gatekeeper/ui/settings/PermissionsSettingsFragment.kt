...
class PermissionsSettingsFragment : SettingsSubFragment() {
    ...
    private fun fillPermissions() {
        ...
        permissionEntries().forEach { entry ->
            val granted = entry.granted()
            val pref = Preference(requireContext()).apply {
                key = entry.key
                title = getString(entry.titleRes)
                summary = statusText(entry)
                // Значок отличает выданное от невыданного с одного взгляда: раньше
    ...
    private fun statusText(entry: PermEntry): CharSequence = if (entry.granted()) {
