...
abstract class SettingsSubFragment : PreferenceFragmentCompat() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        preferenceScreen?.let(PreferenceTitles::allowMultiline)
        // Глубокая ссылка может указывать не только экран, но и строку внутри него.
        activity?.intent
            ?.getStringExtra(io.gatekeeper.ui.SettingsActivity.EXTRA_SCROLL_TO)
            ?.let { key -> findPreference<androidx.preference.Preference>(key)?.let { scrollToPreference(key) } }
    ...
    /**
     * Текст цветом роли темы. Красим только исключение: если покрасить и норму,
     * цвет перестает что-либо значить.
     */
    protected fun tinted(text: CharSequence, colorAttr: Int): CharSequence {
        val value = TypedValue()
        requireContext().theme.resolveAttribute(colorAttr, value, true)
