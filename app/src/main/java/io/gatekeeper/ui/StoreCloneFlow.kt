...
class StoreCloneFlow(private val activity: MainActivity) {
    ...
    private fun pickCloneableStore(
        ...
        // Запускаемость обязательна, а не только наличие APK: на образах без Play
        // пакет com.android.vending -- заглушка LicenseChecker без единой activity
        // (замерено на AVD 16: resolve-activity отвечает "No activity found").
        // Клонировать ее бессмысленно, а пользователь видел предложение склонировать
        // «License checker» и не понимал, что ему предлагают.
        val mainLaunchable = mainApps.associate { wrapper ->
            wrapper.getPackageName() to (
                wrapper.isInstalled() &&
                    !wrapper.getSourceDir().isNullOrBlank() &&
                    wrapper.canLaunch()
                )
        }
        ...
        val (pkg, action) = StoreCloneHint.pickAction(mainLaunchable, workStates) ?: return null
     ...
     * реле для разморозки одного пакета не было и нет -- у действия не было
     * кросс-профильного фильтра, и ветка, которая им пользовалась, оказалась
     * мертвой (удалена). Диалог показывается только при живом serviceWork,
