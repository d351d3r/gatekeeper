...
object StoreCloneHint {
     ...
     * запускаемую копию в личном профиле ([mainLaunchable] == true) и который в
     ...
     */
    fun pickAction(
        mainLaunchable: Map<String, Boolean>,
        workStates: Map<String, Int>,
    ): Pair<String, Int>? =
        CANDIDATES.firstNotNullOfOrNull { pkg ->
            when (workStates[pkg]) {
                WORK_STATE_AVAILABLE -> null
                WORK_STATE_FROZEN -> pkg to ACTION_UNFREEZE
                else -> if (mainLaunchable[pkg] == true) pkg to ACTION_CLONE else null
            }
        }
}
