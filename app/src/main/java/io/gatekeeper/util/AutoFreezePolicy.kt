...
object ScreenLockFreezeMigration {
    ...
    /**
     * Решает, что переносить, и ставит флаг «миграция пройдена»; само применение -- за
     * вызывающим (SettingsManager применяет через сеттеры, чтобы сработали синхронизация
     * в рабочий профиль и подъём FreezeService). Повторные вызовы -- null.
     */
    fun run(storage: LocalStorageManager): Decision? {
        if (storage.getBoolean(LocalStorageManager.PREF_SCREEN_LOCK_FREEZE_MIGRATED)) return null
        val decision = decide(
